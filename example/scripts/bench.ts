...
// bench through the same kernel. Pass a noise id to bench a single algorithm
// across all of its implementations: bun run bench perlin
...
console.log(`${'variant'.padEnd(30)} ${'ms/frame'.padStart(10)} ${'Msamples/s'.padStart(12)}`)
console.log('-'.repeat(54))

const row = (label: string, sample: (x: number, y: number, z: number) => number, scale: number) => {
  const { msPerFrame, msamplesPerSec } = benchJsVariant({ sample }, scale, SIZE, FRAMES)
  console.log(`${label.padEnd(30)} ${msPerFrame.toFixed(2).padStart(10)} ${msamplesPerSec.toFixed(1).padStart(12)}`)
}
