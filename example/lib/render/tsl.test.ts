...
const layer = (over: Partial<LayerConfig> & Pick<LayerConfig, 'noise' | 'scale'>): LayerConfig => ({
  octaves: 1,
  rotate: false,
  style: 'basic',
  blend: 'normal',
...
describe.each(NOISES.map(n => [n.name, n] as const))('%s', (_name, noise) => {
  describe.each(noise.variants.map(v => [v.label, v] as const))('%s', (_label, variant) => {
    test('TSL spec dims are consistent', () => {
      expect(variant.source.tsl?.dim).toBe(variant.dim)
      if (variant.sampleTileable) {
        expect(variant.source.tslTileable?.dim).toBe(variant.dim)
      } else {
        expect(variant.source.tslTileable).toBeUndefined()
      }
    })

    test('composed TSL body evaluates and builds a node graph', () => {
      const cfg = {
        layers: [layer({ noise: variant.source, scale: noise.scale, octaves: 2 })],
        tiled: false,
        size: 512,
      }
    ...
    if (variant.sampleTileable) {
      test('tiled TSL body evaluates and builds a node graph', () => {
        const cfg = { layers: [layer({ noise: variant.source, scale: noise.scale })], tiled: true, size: 512 }
        const effect = evaluate(buildTslBody(cfg))
        expect(effect(TSL.vec2(0.3, 0.7), TSL.float(0.5))).toBeDefined()
      })
    }
...
test('multi-layer TSL body dedupes chunks and builds with blends', () => {
  ...
  const cfg = {
    layers: [
      layer({ noise: (perlin.variants[1] as NoiseVariant).source, scale: perlin.scale, octaves: 3, style: 'ridged' }),
      layer({ noise: (marble.variants[1] as NoiseVariant).source, scale: marble.scale, blend: 'warp', opacity: 0.8 }),
    ],
    tiled: false,
    size: 512,
  }
