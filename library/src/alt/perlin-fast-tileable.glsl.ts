// GLSL counterpart of perlin-fast-tileable.ts. Requires COMMON_GLSL and
// FAST_COMMON_GLSL. The tiling period is baked at 8 cells (the perlin
...
float perlinFastT2(vec2 p) {
  ...
  float d = f.x - f.y;
  float s1 = s - 1.0;
  uint x0 = uint(int(i.x) & 7) * LATTICE_HX;
  uint x1 = uint((int(i.x) + 1) & 7) * LATTICE_HX;
  uint y0 = uint(int(i.y) & 7) * LATTICE_HY;
  uint y1 = uint((int(i.y) + 1) & 7) * LATTICE_HY;
  uint rx0 = x0 ^ (x0 >> 16u);
  uint rx1 = x1 ^ (x1 >> 16u);
